...
int irun, iretfpix;  // added by Mattia (output)
...
void CheckKeyboardCommands(int handle, WaveDumpRun_t *WDrun, WaveDumpRun_t *WDrunFpix, WaveDumpConfig_t *WDcfg, CAEN_DGTZ_BoardInfo_t BoardInfo)  // edited by Mattia (output)
{
    int c = 0;
    if(!kbhit())
        return;
    ...
    if ((c < '9') && (c >= '0')) {
        ...
        case 'P' :
            if (WDrun->ChannelPlotMask == 0)
                ...
                if (WDrun->ContinuousPlot)  // added by Mattia (DAQ usage)
                    printf("Continuous plotting enabled\n");
                else
                    printf("Continuous plotting disabled\n");
            break;
        case 'p' :
            if (WDrun->ChannelPlotMask == 0)
                printf("No channel enabled for plotting\n");
        ...
        case 'w' :
            if (!WDrun->ContinuousWrite)
                WDrun->SingleWrite = 1;
            if (!WDrunFpix->ContinuousWrite)  // added by Mattia (output)
                WDrunFpix->SingleWrite = 1;                
            break;
        case 'W' :
            WDrun->ContinuousWrite ^= 1;
            WDrunFpix->ContinuousWrite ^= 1;  // added by Mattia (output)
            if (WDrun->ContinuousWrite)
...
// vvvvv added by Mattia (output) vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
/*! \brief   Write the FitPix data into the output files
*
*   \param   WDrunFpix Pointer to the WaveDumpRun data structure (specific for FitPix)
*   \param   WDrun Pointer to the WaveDumpRun data structure (the one with the digitizer data)
*   \param   WDcfg Pointer to the WaveDumpConfig data structure
*   \param   EventInfo Pointer to the EventInfo data structure
*   \param   EventFpix Pointer to the s_fitpix_out data structure
*/
int WriteOutputFilesFitPix(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, WaveDumpRun_t *WDrunFpix, CAEN_DGTZ_EventInfo_t *EventInfo, s_fitpix_out_t *EventFpix)
{
    int ch, j;

    for (ch = 0; ch < MAX_DEV; ch++) {
        if (!WDrunFpix->fout[ch]) {
            char fname[100];
            sprintf(fname, "%s/wave%010d_fitpix%d.txt",path, EventInfo->EventCounter, ch);
            if ((WDrunFpix->fout[ch] = fopen(fname, "w")) == NULL)
                return -1;
        }
        if( WDcfg->OutFileFlags & OFF_HEADER) {
            fprintf(WDrunFpix->fout[ch], "Event Number: %d\n", EventInfo->EventCounter);
            fprintf(WDrunFpix->fout[ch], "Trigger Time Stamp: %u\n", EventInfo->TriggerTimeTag);
            fprintf(WDrunFpix->fout[ch], "FitPix readout UTC time: %f\n", EventFpix->fpixUTCsec);
            fprintf(WDrunFpix->fout[ch], "FitPix DAQ UTC time: %f\n", EventFpix->machineUTCsec);
            fprintf(WDrunFpix->fout[ch], "FitPix readout-DAQ jitter: %f\n", EventFpix->jitterUTCsec);
            fprintf(WDrunFpix->fout[ch], "FitPix Nr.: %d\n", ch);
            fprintf(WDrunFpix->fout[ch], "FitPix Key: %s\n", EventFpix->devKey[ch]);
            fprintf(WDrunFpix->fout[ch], "FitPix DAQ Type: 0x%X\n", EventFpix->TypeOfPack[ch]);
            fprintf(WDrunFpix->fout[ch], "FitPix Frame Nr.: %d\n", EventFpix->nFrame[ch]);
            fprintf(WDrunFpix->fout[ch], "Nr. of Pixels: %d\n", EventFpix->fpixLength[ch]);
        }
        for(j=0; j<EventFpix->fpixLength[ch]; j++) {
            fprintf(WDrunFpix->fout[ch], "%d\n", EventFpix->fpix[ch][j]);
        }
        if (WDrunFpix->ContinuousWrite || WDrunFpix->SingleWrite) {
            if (EventInfo->EventCounter%1000 == 0) {
                fclose(WDrunFpix->fout[ch]);
                WDrunFpix->fout[ch]= NULL;
            }
        }
    }
    return 0;
...
*   \param   EventFpix Pointer to the s_fitpix_out data structure
*/
int WriteOutputFilesFitPixx742(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, WaveDumpRun_t *WDrunFpix, CAEN_DGTZ_EventInfo_t *EventInfo, CAEN_DGTZ_X742_EVENT_t *Event, s_fitpix_out_t *EventFpix)
{
    int ch, j;
    ...
    for (ch = 0; ch < MAX_DEV; ch++) {
        ...
        if( WDcfg->OutFileFlags & OFF_HEADER) {
            fprintf(WDrunFpix->fout[ch], "Event Number: %d\n", EventInfo->EventCounter);
            fprintf(WDrunFpix->fout[ch], "Trigger Time Stamp: %u\n", Event->DataGroup[0].TriggerTimeTag);  // TEMPORARY FIX!
            fprintf(WDrunFpix->fout[ch], "FitPix readout UTC time: %f\n", EventFpix->fpixUTCsec);
            fprintf(WDrunFpix->fout[ch], "FitPix DAQ UTC time: %f\n", EventFpix->machineUTCsec);
            fprintf(WDrunFpix->fout[ch], "FitPix readout-DAQ jitter: %f\n", EventFpix->jitterUTCsec);
            fprintf(WDrunFpix->fout[ch], "FitPix Nr.: %d\n", ch);
...
// vvvvv duplicated by Mattia (output) vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
/*! \brief   Write the event data and some FitPix info into the output files
*
*   \param   WDrun Pointer to the WaveDumpRun data structure
*   \param   WDcfg Pointer to the WaveDumpConfig data structure
*   \param   EventInfo Pointer to the EventInfo data structure
...
*   \param   EventFpix Pointer to the s_fitpix_out data structure
*/
int WriteOutputFilesPlusFitPixHeader(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, void *Event, s_fitpix_out_t *EventFpix)
{
    int ch, j, ns;
    int chfpix;  // added by Mattia (output)
    CAEN_DGTZ_UINT16_EVENT_t  *Event16 = NULL;
    CAEN_DGTZ_UINT8_EVENT_t   *Event8 = NULL;
    ...
    for (ch = 0; ch < WDcfg->Nch; ch++) {
        ...
        if( WDcfg->OutFileFlags& OFF_BINARY) {
            // Binary file format
            uint32_t BinHeader[6];  // STILL NEED TO IMPLEMENT FITPIX HEADER FOR BINARY!
            ...
            if (!WDrun->fout[ch]) {
                ...
                sprintf(fname, "%s/wave%010d_0_%d.dat",path, EventInfo->EventCounter, ch);  // edited by Mattia (output)
            ...
            if (!WDrun->fout[ch]) {
                ...
                sprintf(fname, "%s/wave%010d_0_%d.txt",path, EventInfo->EventCounter, ch);  // edited by Mattia (output)
            ...
            if( WDcfg->OutFileFlags & OFF_HEADER) {
                ...
                for (chfpix=0; chfpix < MAX_DEV; chfpix++) {
                    fprintf(WDrun->fout[ch], "FitPix readout UTC time: %f\n", EventFpix->fpixUTCsec);
                    fprintf(WDrun->fout[ch], "FitPix DAQ UTC time: %f\n", EventFpix->machineUTCsec);
                    fprintf(WDrun->fout[ch], "FitPix readout-DAQ jitter: %f\n", EventFpix->jitterUTCsec);
                    fprintf(WDrun->fout[ch], "FitPix %d Key: %s\n", chfpix, EventFpix->devKey[chfpix]);
                    fprintf(WDrun->fout[ch], "FitPix %d DAQ Type: 0x%X\n", chfpix, EventFpix->TypeOfPack[chfpix]);
                    fprintf(WDrun->fout[ch], "FitPix %d Frame Nr.: %d\n", chfpix, EventFpix->nFrame[chfpix]);
                    fprintf(WDrun->fout[ch], "FitPix %d Nr. of Pixels: %d\n", chfpix, EventFpix->fpixLength[chfpix]);
                }
            }
            for(j=0; j<Size; j++) {
                if (WDcfg->Nbit == 8)
        ...
        if (WDrun->ContinuousWrite || WDrun->SingleWrite) {  // edited by Mattia (output)
            //fclose(WDrun->fout[ch]);
			//WDrun->fout[ch]= NULL;
			if (EventInfo->EventCounter%1000 == 0) {  // edited by Mattia (output)
				fclose(WDrun->fout[ch]);
				WDrun->fout[ch]= NULL;
			}
        }
...
// vvvvv duplicated by Mattia (output) vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
/*! \brief   Write the event data on x742 boards and some FitPix info into the output files into the output files
*
*   \param   WDrun Pointer to the WaveDumpRun data structure
*   \param   WDcfg Pointer to the WaveDumpConfig data structure
*   \param   EventInfo Pointer to the EventInfo data structure
...
*   \param   EventFpix Pointer to the s_fitpix_out data structure
*/
int WriteOutputFilesx742PlusFitPixHeader(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, CAEN_DGTZ_X742_EVENT_t *Event, s_fitpix_out_t *EventFpix)
{
    int gr,ch, j, ns;
    int chfpix;  // added by Mattia (output)
    ...
    for (gr=0;gr<(WDcfg->Nch/8);gr++) {
        if (Event->GrPresent[gr]) {
            for(ch=0; ch<9; ch++) {
                ...
                if( WDcfg->OutFileFlags& OFF_BINARY) {
                    // Binary file format
                    uint32_t BinHeader[8];  // STILL NEED TO IMPLEMENT FITPIX HEADER FOR BINARY!
                    ...
                    if (!WDrun->fout[(gr*9+ch)]) {
                        ...
                        if ((gr*9+ch) == 8) {
                            ...
                            sprintf(fname, "%s/TR%010d_0.dat",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_0");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 17) {
                            ...
                            sprintf(fname, "%s/TR%010d_1.dat",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_1");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 26) {
                            ...
                            sprintf(fname, "%s/TR%010d_2.dat",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_2");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 35) {
                            ...
                            sprintf(fname, "%s/TR%010d_3.dat",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_3");
                            flag = 1;
                        }
                        else 	{
	                        //sprintf(fname, "%swave_%d.dat",path, (gr*8)+ch);
	                        sprintf(fname, "%s/wave%010d_%d_%d.dat",path, EventInfo->EventCounter, gr, ch);  // edited by Mattia (output)
	                        flag = 0;
                        }
                        //if ((WDrun->fout[(gr*9+ch)] = fopen(fname, "w")) == NULL)
                        //    return -1;
						if (WDcfg->EnableIthChannel[gr][ch]==1) {  // edited by Mattia (output)
							if ((EventInfo->EventCounter%1000 == 0) || (WDrun->fout[(gr*9+ch)]==NULL)) {
				                if ((WDrun->fout[(gr*9+ch)] = fopen(fname, "w")) == NULL)
				                    return -1;
							}
						} else {continue;}
                    ...
                    if (!WDrun->fout[(gr*9+ch)]) {
                        ...
                        if ((gr*9+ch) == 8) {
                            ...
                            //sprintf(trname,"TR_0_0");
                            sprintf(fname, "%s/TR%010d_0.txt",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_0");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 17) {
                            ...
                            //sprintf(trname,"TR_0_1");
                            sprintf(fname, "%s/TR%010d_1.txt",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_1");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 26) {
                            ...
                            //sprintf(trname,"TR_1_0");
                            sprintf(fname, "%s/TR%010d_2.txt",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_2");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 35) {
                            ...
                            //sprintf(trname,"TR_1_1");
                            sprintf(fname, "%s/TR%010d_3.txt",path, EventInfo->EventCounter);  // edited by Mattia (output)
                            sprintf(trname,"TR_3");
                            flag = 1;
                        }
                        else 	{
	                        //sprintf(fname, "%swave_%d.txt",path, (gr*8)+ch);
		                    sprintf(fname, "%s/wave%010d_%d_%d.txt",path, EventInfo->EventCounter, gr, ch);  // edited by Mattia (output)
		                    flag = 0;
                        }
                        //if ((WDrun->fout[(gr*9+ch)] = fopen(fname, "w")) == NULL)
                        //    return -1;
					...
					if (WDcfg->EnableIthChannel[gr][ch]==1) {  // added by Mattia (output)
		                if( WDcfg->OutFileFlags & OFF_HEADER) {
		                    // Write the Channel Header
                            ...
                            // added by Mattia (output)
                            for (chfpix=0; chfpix < MAX_DEV; chfpix++) {
                                fprintf(WDrun->fout[(gr*9+ch)], "FitPix readout UTC time: %f\n", EventFpix->fpixUTCsec);
                                fprintf(WDrun->fout[(gr*9+ch)], "FitPix DAQ UTC time: %f\n", EventFpix->machineUTCsec);
                                fprintf(WDrun->fout[(gr*9+ch)], "FitPix readout-DAQ jitter: %f\n", EventFpix->jitterUTCsec);
                                fprintf(WDrun->fout[(gr*9+ch)], "FitPix %d Key: %s\n", chfpix, EventFpix->devKey[chfpix]);
                                fprintf(WDrun->fout[(gr*9+ch)], "FitPix %d DAQ Type: 0x%X\n", chfpix, EventFpix->TypeOfPack[chfpix]);
                                fprintf(WDrun->fout[(gr*9+ch)], "FitPix %d Frame Nr.: %d\n", chfpix, EventFpix->nFrame[chfpix]);
                                fprintf(WDrun->fout[(gr*9+ch)], "FitPix %d Nr. of Pixels: %d\n", chfpix, EventFpix->fpixLength[chfpix]);
                            }

		                }
		                for(j=0; j<Size; j++) {
		                    fprintf(WDrun->fout[(gr*9+ch)], "%f\n", Event->DataGroup[gr].DataChannel[ch][j]);  // for float samples
		                    //fprintf(WDrun->fout[(gr*9+ch)], "%d\n", (int)Event->DataGroup[gr].DataChannel[ch][j]);  // for int samples
		                }
					}
                }
                //if (WDrun->SingleWrite) {
                if (WDrun->ContinuousWrite || WDrun->SingleWrite) {  // edited by Mattia (output)
                    //fclose(WDrun->fout[(gr*9+ch)]);
					//WDrun->fout[(gr*9+ch)]= NULL;
					if (WDcfg->EnableIthChannel[gr][ch]==1) {
						if (EventInfo->EventCounter%1000 == 0) {
							fclose(WDrun->fout[(gr*9+ch)]);
							WDrun->fout[(gr*9+ch)]= NULL;
						}
					} else {continue;}  // edited by Mattia (output)
                }
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    WaveDumpRun_t      WDrun;
    WaveDumpRun_t      WDrunFpix;  // added by Mattia (output)
    CAEN_DGTZ_ErrorCode ret = CAEN_DGTZ_Success;
    int  handle = -1;
    ERROR_CODES ErrCode= ERR_NONE;
    int i, ch, Nb=0, Ne=0;
    int j;  // added by Mattia (DAQ usage)
    ...
    printf("**************************************************************\n");
    //printf("                        Wave Dump %s\n", WaveDump_Release);
    printf("     Wave Dump %s MATTIA'S EDITION\n", WaveDump_Release);  // edited by Mattia (DAQ usage)
    printf("**************************************************************\n");
    ...
    memset(&WDrunFpix, 0, sizeof(WDrunFpix));  // added by Mattia (output)
    ...
    // added by Mattia (output)
    #ifndef WIN32
        sprintf(path, WDcfg.OutFilePath);
        mkdir(path, 0777);

        // remove this part for no run subfolders
        irun = (int)time(NULL);
        char path2[128];
        sprintf(path2, "%d", irun);
        strcat(path, path2);

        printf("Data will go to %s\n", path);

    #endif

    // added by Mattia (FitPix)
    int s_fitpix_mcsuccess = 0;
    #ifdef RUNFITPIX
        printf("FitPix: adding MC server... ");
	    s_fitpix_initVarsLoop();
        s_fitpix_mcsuccess = s_fitpix_initMc();
    #else
        printf("FitPix not included\n");
    #endif
...
Restart:
    ...
    if(ReloadCfgStatus > 0) {
        ...
        // added by Mattia (output)
            ...
            strcpy(path3, path);
            strcat(path3, "/");
            strcat(path3, "confdump.txt");
            printf("Copying conf file to %s\n", path3);
            sprintf(command,"cp -f %s %s", ConfigFileName, path3);
            if ((fp=popen(command,"r")) != NULL)
              pclose(fp);
        #endif
	    char TablePathName[128];
	    sprintf(TablePathName, "%s/X742Table", path);
        ...
        if(BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE && (ReloadCfgStatus & (0x1 << CFGRELOAD_CORRTABLES_BIT)) ) {
            if(WDcfg.useCorrections != -1) { // Use Manual Corrections
                ...
                if(WDcfg.UseManualTables != -1) { // The user wants to use some custom tables
                    ...
                    // Save to file the Tables read from flash // moved and edited by Mattia (output)
                    if (WDcfg.useCorrections != 0) {
                        GroupMask = ((0x1<<WDcfg.MaxGroupNumber)-1);
		                SaveCorrectionTables(TablePathName, GroupMask, X742Tables);
                    }
                ...
                if ((ret = CAEN_DGTZ_EnableDRS4Correction(handle)) != CAEN_DGTZ_Success)
                    goto QuitProgram;
                if ((ret = CAEN_DGTZ_GetCorrectionTables(handle, WDcfg.DRS4Frequency, (void*)X742Tables)) != CAEN_DGTZ_Success) // added by Mattia (output)
                    goto QuitProgram; // added by Mattia (output)
                SaveCorrectionTables(TablePathName, -1, X742Tables); // added by Mattia (output)
            }
        }
    }
    ...
    WDrun.Restart = 0;
    WDrunFpix.Restart = 0;  // added by Mattia (output)
    ...
    while(!WDrun.Quit) {		
        ...
        CheckKeyboardCommands(handle, &WDrun, &WDrunFpix, &WDcfg, BoardInfo);
        if (WDrun.Restart) {
            CAEN_DGTZ_SWStopAcquisition(handle);
            CAEN_DGTZ_FreeReadoutBuffer(&buffer);
            ClosePlotter();
	...
	utcMachineTimeNs = currentTime.tv_nsec;
        ...
        if (FPIX_BYPASS_TRG && FPIX_BYPASS_SLEEP)            
            sleep(1);
        if ((BufferSize>0) || FPIX_BYPASS_TRG) {
            if ((s_fitpix_mcsuccess)) {
                s_fpix_out = s_fitpix_read();
                utcJitter = s_fpix_out.fpixUTCsec - utcMachineTime;
                s_fpix_out.machineUTCsec = utcMachineTime;
                s_fpix_out.jitterUTCsec = utcJitter;
                if ((FPIX_PRINT_OUTSIDE)) {
                    if ((s_fpix_out.fpixUTCsec!=0)) {
                        printf("\n+++ NEW EVENT [OUT-OF-FUNCTION] +++\n");
                        printf("Device %s, new event (on time %f) nr. %d\n", s_fpix_out.devKey[FPIX_PRINT_DEV],s_fpix_out.fpixUTCsec,s_fpix_out.k);
                        printf("Jitter wrt. DAQ time (%f) is %f\n", s_fpix_out.machineUTCsec, s_fpix_out.jitterUTCsec);
                        printf("TypeOfPack %x, hOffset %d, nFrame %d, fpixLength %d\n", s_fpix_out.TypeOfPack[FPIX_PRINT_DEV], s_fpix_out.hOffset[FPIX_PRINT_DEV], s_fpix_out.nFrame[FPIX_PRINT_DEV], s_fpix_out.fpixLength[FPIX_PRINT_DEV]);
                        for (i = 0; i < s_fpix_out.fpixLength[FPIX_PRINT_DEV]; i++) {
                            printf("Device %s, %d-th pixel: %d\n",s_fpix_out.devKey[FPIX_PRINT_DEV],i,s_fpix_out.fpix[FPIX_PRINT_DEV][i]);
                        }
                        printf("+++ EVENT END [OUT-OF-FUNCTION] +++\n\n");
                    }else{
                        printf("\n+ NO NEW EVENTS [OUT-OF-FUNCTION] +\n\n");
                    }
                }
            }else{
                s_fpix_out.fpixUTCsec = 0.0;
                s_fpix_out.machineUTCsec = 0.0;
                s_fpix_out.jitterUTCsec = 0.0;
                s_fpix_out.k = 0;
                s_fpix_out.nDevs = 0;
                for (i=0; i<MAX_DEV; i++) {
                    s_fpix_out.devId[i] = devId0[i];
                    s_fpix_out.devKey[i] = devKey0[i];
                    s_fpix_out.TypeOfPack[i] = TypeOfPack0[i];
                    s_fpix_out.hOffset[i] = hOffset0[i];
                    s_fpix_out.nFrame[i] = nFrame0[i];
                    s_fpix_out.fpixLength[i] = fpixLength0[i];
                    for (j = 0; j < SINGLE_CHIP_PIXSIZE; j++) {
                        s_fpix_out.fpix[i][j] = fpix0[i][j];
                    }
                }
                if ((FPIX_PRINT_OUTSIDE)) {
                    printf("\n+++ NEW EVENT [OUT-OF-FUNCTION] +++\n");
                    printf("++++ NO NEW EVENT, USING DUMMY ++++\n");
                    printf("Device %s, new event (on time %f) nr. %d\n", s_fpix_out.devKey[FPIX_PRINT_DEV],s_fpix_out.fpixUTCsec,s_fpix_out.k);
                    printf("Jitter wrt. DAQ time (%f) is %f\n", s_fpix_out.machineUTCsec, s_fpix_out.jitterUTCsec);
                    printf("TypeOfPack %x, hOffset %d, nFrame %d, fpixLength %d\n", s_fpix_out.TypeOfPack[FPIX_PRINT_DEV], s_fpix_out.hOffset[FPIX_PRINT_DEV], s_fpix_out.nFrame[FPIX_PRINT_DEV], s_fpix_out.fpixLength[FPIX_PRINT_DEV]);
                    printf("+++ EVENT END [OUT-OF-FUNCTION] +++\n\n");
        ...
        for(i = 0; i < (int)NumEvents; i++) {
            ...
            if (WDcfg.Nbit == 8) 
                ...
                if (WDrun.ContinuousWrite || WDrun.SingleWrite) {
                    ...
                    #ifndef WIN32  // added by Mattia (output)
					    mkdir(path, 0777);
                    #endif
			...
			if (BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE)
			    ...
			    iretfpix = WriteOutputFilesFitPix(&WDcfg, &WDrun, &WDrunFpix, &EventInfo, &s_fpix_out);
                    ...
                    //}
                    if (BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE) {	// edited by Mattia (output)
                        if (FPIX_WRITE)
                            ret = WriteOutputFilesx742PlusFitPixHeader(&WDcfg, &WDrun, &EventInfo, Event742, &s_fpix_out);
                        else
                            ret = WriteOutputFilesx742(&WDcfg, &WDrun, &EventInfo, Event742);
                    }
                    else if (WDcfg.Nbit == 8) {
