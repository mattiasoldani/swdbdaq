...
// vvvvv added by Mattia (DAQ usage) vvvvvvvvvvvvvvvvvv

        ///xx742 only

        // list of channels to be written in output files
		// note: careful, the running plot works even for channels disabled here!
        if (strstr(str, "ENABLE_ITH_CHANNEL")!=NULL)
        {
            float eich[9];
            read = fscanf(f_ini, "%f,%f,%f,%f,%f,%f,%f,%f,%f", &eich[0], &eich[1], &eich[2], &eich[3], &eich[4], &eich[5], &eich[6], &eich[7], &eich[8]);
	
            for(i=0; i<9; i++)
			{				
				val = (int)(eich[i]);
				WDcfg->EnableIthChannel[ch][i]=val;
            }
			continue;
		}

        // path in which to write all output
		if (strstr(str, "OUTPUT_FILE_PATH")!=NULL)
        {    
            read = fscanf(f_ini, "%s", WDcfg->OutFilePath);
            continue;
        }
