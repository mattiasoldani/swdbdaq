...
#include "s_fitpix.h"  // added by Mattia (FitPix)
#include <stdbool.h>
...
int WriteOutputFiles(WaveDumpConfig_t *WDcfg, WaveDumpRun_t *WDrun, CAEN_DGTZ_EventInfo_t *EventInfo, void *Event)
{
    ...
    for (ch = 0; ch < WDcfg->Nch; ch++) {
        ...
        if( WDcfg->OutFileFlags& OFF_BINARY) {
            ...
            if (!WDrun->fout[ch]) {
                char fname[100];
                //sprintf(fname, "%swave%d.dat", path,ch);  // edited by Mattia
                sprintf(fname, "%s/wave%010d_0_%d.dat",path, EventInfo->EventCounter, ch);
            ...
            if (!WDrun->fout[ch]) {
                char fname[100];
                //sprintf(fname, "%swave%d.txt", path, ch);  // edited by Mattia
                sprintf(fname, "%s/wave%010d_0_%d.txt",path, EventInfo->EventCounter, ch);
        ...
        if (WDrun->ContinuousWrite || WDrun->SingleWrite) {  // edited by Mattia
            //fclose(WDrun->fout[ch]);
			//WDrun->fout[ch]= NULL;
			if (EventInfo->EventCounter%1000 == 1) {
				fclose(WDrun->fout[ch]);
				WDrun->fout[ch]= NULL;
			}  // edited by Mattia
        }
        ...
        if (Event->GrPresent[gr]) {
            for(ch=0; ch<9; ch++) {
                ...
                if( WDcfg->OutFileFlags& OFF_BINARY) {
                    ...
                    if (!WDrun->fout[(gr*9+ch)]) {
                        ...
                        if ((gr*9+ch) == 8) {
                            ...
                            sprintf(fname, "%s/TR%010d_0.dat",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_0");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 17) {
                            ...
                            sprintf(fname, "%s/TR%010d_1.dat",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_1");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 26) {
                            ...
                            sprintf(fname, "%s/TR%010d_2.dat",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_2");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 35) {
                            ...
                            sprintf(fname, "%s/TR%010d_3.dat",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_3");
                            flag = 1;
                        }
                        else 	{
	                        //sprintf(fname, "%swave_%d.dat",path, (gr*8)+ch);
	                        sprintf(fname, "%s/wave%010d_%d_%d.dat",path, EventInfo->EventCounter, gr, ch);  // edited by Mattia
	                        flag = 0;
                        }
                        //if ((WDrun->fout[(gr*9+ch)] = fopen(fname, "w")) == NULL)
                        //    return -1;
						if (WDcfg->EnableIthChannel[gr][ch]==1) {  // edited by Mattia
							if ((EventInfo->EventCounter%1000 == 1) || (WDrun->fout[(gr*9+ch)]==NULL)) {
				                if ((WDrun->fout[(gr*9+ch)] = fopen(fname, "w")) == NULL)
				                    return -1;
							}
						} else {continue;}
                    ...
                    if (!WDrun->fout[(gr*9+ch)]) {
                        ...
                        if ((gr*9+ch) == 8) {
                            ...
                            //sprintf(trname,"TR_0_0");
                            sprintf(fname, "%s/TR%010d_0.txt",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_0");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 17) {
                            ...
                            //sprintf(trname,"TR_0_1");
                            sprintf(fname, "%s/TR%010d_1.txt",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_1");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 26) {
                            ...
                            //sprintf(trname,"TR_1_0");
                            sprintf(fname, "%s/TR%010d_2.txt",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_2");
                            flag = 1;
                        }
                        else if ((gr*9+ch) == 35) {
                            ...
                            //sprintf(trname,"TR_1_1");
                            sprintf(fname, "%s/TR%010d_3.txt",path, EventInfo->EventCounter);  // edited by Mattia
                            sprintf(trname,"TR_3");
                            flag = 1;
                        }
                        else 	{
	                        //sprintf(fname, "%swave_%d.txt",path, (gr*8)+ch);
		                    sprintf(fname, "%s/wave%010d_%d_%d.txt",path, EventInfo->EventCounter, gr, ch);  // edited by Mattia
		                    flag = 0;
                        }
                        //if ((WDrun->fout[(gr*9+ch)] = fopen(fname, "w")) == NULL)
                        //    return -1;
					...
					if (WDcfg->EnableIthChannel[gr][ch]==1) {  // added by Mattia
		                if( WDcfg->OutFileFlags & OFF_HEADER) {
		                    // Write the Channel Header
		                ...
		                for(j=0; j<Size; j++) {
		                    ...
		                    //fprintf(WDrun->fout[(gr*9+ch)], "%d\n", (int)Event->DataGroup[gr].DataChannel[ch][j]);  // edited by Mattia (to write int instead of float)
		                }
					}
                }
                //if (WDrun->SingleWrite) {
                if (WDrun->ContinuousWrite || WDrun->SingleWrite) {  // edited by Mattia
					...
					} else {continue;}  // edited by Mattia
                }
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    printf("**************************************************************\n");
    //printf("                        Wave Dump %s\n", WaveDump_Release);
    printf("     Wave Dump %s MATTIA'S EDITION\n", WaveDump_Release);  // modified by Mattia
    printf("**************************************************************\n");
    ...
    #define RUNFITPIX // comment to exclude FitPix entirely
    bool s_fitpix_mcsuccess = false;
    #ifdef RUNFITPIX
        ...
        if(BoardInfo.FamilyCode == CAEN_DGTZ_XX742_FAMILY_CODE && (ReloadCfgStatus & (0x1 << CFGRELOAD_CORRTABLES_BIT)) ) {
            if(WDcfg.useCorrections != -1) { // Use Manual Corrections
                ...
                if(WDcfg.UseManualTables != -1) { // The user wants to use some custom tables
                    ...
                    // Save to file the Tables read from flash // moved and edited by Mattia
                ...
                if ((ret = CAEN_DGTZ_EnableDRS4Correction(handle)) != CAEN_DGTZ_Success)
                    goto QuitProgram;
                if ((ret = CAEN_DGTZ_GetCorrectionTables(handle, WDcfg.DRS4Frequency, (void*)X742Tables)) != CAEN_DGTZ_Success) // added by Mattia
                    goto QuitProgram; // added by Mattia
                SaveCorrectionTables(TablePathName, -1, X742Tables); // added by Mattia
            }
        }
    }
...
InterruptTimeout:
        ...
        // added by Mattia (FitPix)
        if ((BufferSize>0) && (s_fitpix_mcsuccess)) {
	//if ((s_fitpix_mcsuccess)) {
            s_fitpix_out_t s_fitpix_outdata;
            s_fitpix_outdata = s_fitpix_read();
	    if ((s_fitpix_outdata.fpixUTCsec!=0)){
		for (i = 0; i < s_fitpix_outdata.fpixLength; i++) {
		printf("outside: %d %d\n",s_fitpix_outdata.fpix[i], s_fitpix_outdata.fpixLength);
		}
        }}
